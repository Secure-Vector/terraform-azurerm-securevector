...
> **One `terraform apply` → a live, HTTPS SecureVector engine in your own Azure
> subscription, in ~5 minutes.** The turnkey *server* companion to the
...
data live in *your* subscription, in *your* tenant — nothing leaves.
...
Azure Container Apps gives **managed HTTPS, a public FQDN, and scale-to-zero**
out of the box — the closest Azure analog to GCP Cloud Run, so the whole
deployment is a handful of resources with no VNet, load balancer, or manual TLS.
Persistence rides on an **Azure Files** share mounted into the engine's data dir.

```
terraform apply -var="location=eastus"
#
# Outputs:
#   dashboard_url   = "https://securevector.<region>.azurecontainerapps.io"   # live, public, managed TLS
...
### Prerequisites
- An Azure subscription and `az` authenticated (Terraform uses the Azure CLI /
  env / managed identity for auth).
- Terraform `>= 1.5` (or OpenTofu).
- Permission to create Container Apps, Log Analytics, and Storage resources.
...
|---|---|---|
| What you get | Your own engine doing **local, device-level** detection — local rules + the **Guardian ML** model — running entirely in your subscription. | Everything in Option 1, **plus** the SecureVector cloud: org **fleet** management, **policy sync**, and the cloud's **advanced ML / enhanced `/analyze`**. |
| Needs | Just an Azure subscription. No SecureVector account. | **Requires a SecureVector account (sign up).** An `svet_*` enrollment token (and/or `svpk_*` key); cloud tiers/billing apply. |
...
Just a location: scale-to-zero Container App, a public HTTPS FQDN, local
detection, and a clean `terraform destroy`. This is the
[`examples/free-tier`](examples/free-tier) example.
...
> (internal-only ingress) / `ingress_cidrs` (CIDR allowlist).
...
```hcl
module "securevector" {
  source  = "Secure-Vector/securevector/azurerm"
  ...
  # allow_unauthenticated = false               # or/also internal-only ingress
}
...
`source = "github.com/Secure-Vector/terraform-azurerm-securevector"`.
...
| `name` | string | `securevector` | Container App / resource base name (≤32). |
| `create_resource_group` | bool | `true` | Create the RG, or use an existing one. |
| `resource_group_name` | string | `""` | RG name (default `<name>-rg`). |
...
| `cpu` / `memory` | number / string | `0.5` / `1Gi` | Per-replica size (valid Container Apps combo). |
...
| `allow_unauthenticated` | bool | `true` | Public FQDN (external ingress). `false` = internal-only. |
| `ingress_cidrs` | list(string) | `[]` | Optional CIDR allowlist (`ip_security_restriction`). |
| `ingress_token` | string (sensitive) | `""` | App-layer inbound gate → `SECUREVECTOR_INGRESS_TOKEN` (Container App secret). |
...
| `enable_persistence` | bool | `true` | Mount an Azure Files share for the audit hash-chain. |
...
| `storage_account_name` | string | `""` | Override the (globally-unique) storage account name. |
| `storage_share_quota_gb` | number | `10` | Azure Files share quota (GiB). |
| `log_retention_days` | number | `30` | Log Analytics retention. |
| `tags` | map(string) | `{}` | Tags on created resources. |
| `extra_env` | map(string) | `{}` | Extra (non-sensitive) container env vars. |
...
| `fqdn` | Container App ingress FQDN. |
| `container_app_name` / `resource_group` / `location` | Deployed identity. |
| `persistence_storage_account` | Audit-chain storage account (null if persistence off). |
...
those leave `ingress_token` unset or use internal-only ingress. (Plugin list
...
|---|---|---|---|
| **Remote analyze** (client → engine) | inbound | `ingress_token` → `SECUREVECTOR_INGRESS_TOKEN` | Engine requires `Authorization: Bearer`/`X-Api-Key` when set (fail-open when unset). Header-capable clients (OpenClaw, curl) work today; SDK/JS-hook forwarding rolling out (#182). Or use internal-only ingress. |
...
> For production, source `securevector_api_key` / `cloud_connect_token` from Azure
> Key Vault rather than tfvars (roadmap; see wiki open questions).

## Persistence

`enable_persistence = true` provisions a Standard LRS storage account + Azure
Files share, links it to the Container Apps environment, and mounts it at the
engine's data dir. The app has **no data-dir env override** — it writes to
`$HOME/.local/share/securevector/threat-monitor` — so `persistence_mount_path`
must equal that path in the published image. For a throwaway trial, set
...
Removes the Container App, environment, Log Analytics workspace, storage account,
and (when created) the resource group. No leftover billable resources.
