...
variable "ingress_token" {
  description = "App-layer inbound credential -> SECUREVECTOR_INGRESS_TOKEN (stored as a Container App secret). Enforced from engine v4.9.0+: when set, a v4.9.0+ engine requires it on every request (Authorization: Bearer <token> or X-Api-Key: <token>); /health stays open for probes. Header-capable clients (OpenClaw, curl) can pass it today; SDK/JS-hook client-side forwarding is rolling out (#182). On engines older than v4.9.0 the env var is set but NOT enforced — rely on network gating until you pin a v4.9.0+ image. Empty = no app-layer gate."
  type        = string
  default     = ""
  sensitive   = true
}
